...
```
  ...
  + optional config          │
  + optional alignment       │
         ├──► Java BioFabric ──► golden .noa, .eda, .bif, .scores  (pre-generated)
         │
         └──► biofabric-rs   ──► output .noa, .eda, .bif, .scores
                                               │
                                         byte-for-byte diff
```

Golden files are pre-generated reference outputs from the original Java
BioFabric, stored under `tests/parity/goldens/`. The Rust implementation
must produce identical output for each test case.
